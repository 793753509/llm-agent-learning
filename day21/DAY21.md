# Day 21：让程序替你检查“刚才演示成功的事，现在还成功吗？”
...
| 内容 | 来源与用途 |
|---|---|
| [verify.py](verify.py) | 人工编写的验收用例和断言，这次没有单独的 cases.json |
| 临时数据库、故障样例 | verify 运行时创建，用于执行检查，完成后清理 |
| `reports/local.json`、`reports/local.md` | 同一次真实执行生成的两种报告格式，重复运行会更新 |

`local` 表示本地默认分支，`hybrid` 报告对应启用向量的分支；Markdown 报告由同一份 JSON 数据整理而来。
代码中 `verify()` 执行检查，`write_report()` 保存两种格式并自动创建报告目录。
...
```text
day21/reports/local.json  完整机器可读结果
day21/reports/local.md    适合直接阅读的结果
```
...
Day 18/20 的 curl 实验补上实际监听与 HTTP 通信的观察。
...
这覆盖 Day 11 讨论的“写入已成功，完成快照尚未保存”的关键风险。
...
真实项目会把已确认的业务要求写成自动检查，在改代码或发布前执行；涉及模型语义的部分另用业务用例和人工核对。报告应保存本次配置与时间，并能追到实际输入和失败记录。先覆盖重要路径，再根据真实失败补用例。
...
[上一课：Day 20](../day20/DAY20.md) · [下一课：Day 22](../day22/DAY22.md)
