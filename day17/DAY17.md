...
```bash
uv run --group api uvicorn day17.api:app --host 127.0.0.1 --port 8000
```

命令持续运行是正常的；按 Ctrl+C 停止。
`day17.api:app` 的意思是“导入 day17/api.py 中的 app 对象”。
...
Day 19 会把它换成最终项目入口，因此 API 不必重写一遍。
