...
| 流程快照 | 工单草稿等批准，还没写入 | Day 10，checkpoint |
...
```bash
uv run python -m day12.memory remember --user alice --key style --value 简短
uv run python -m day12.memory list --user alice
uv run python -m day12.memory list --user bob
```

前两次应显示 style=简短；bob 的列表应为空（假设你还没给 bob 存过）。
每条命令是独立进程。仍能读到，说明不是仅保存在 Python 变量中。

文件在 `day12/data/memory.sqlite`，是你的本地文件，不在模型服务商服务器。
...
```bash
uv run python -m day12.memory remember --user alice --key style --value 详细
```
...
```bash
uv run python -m day12.memory remember --key password --value abc
```
...
```bash
uv run python -m day12.memory forget --user alice --key style
uv run python -m day12.memory list --user alice
```
...
```bash
uv run python -m day12.memory remember --user alice --key style --value 简短 --ttl 1
```
...
这一步只是从本地读数据，模型还看不到。
如果想影响回答，应用必须选择相关偏好，放入本次模型输入；或者像 Day 19 那样用偏好控制显示条数。
